...
SELECT DISTINCT region AS '地方'
FROM countries;

-- 問15
-- 国名と人口を以下のように表示させてください。シングルクォートに注意してください。
-- 「Arubaの人口は103000人です」
SELECT CONCAT (name,'の人口は',population,'人 です') AS 'POPULATION'
FROM countries;
...
SELECT region AS '地方', AVG(life_expectancy) AS '平均寿命', AVG(population) AS '平均人口'
FROM countries
GROUP BY region;

-- 問22
-- 全ての地方の最長寿命、最大人口を表示してください。(NULLも表示)
SELECT region AS '地方', MAX(life_expectancy) AS '最大寿命', MAX(population) AS '最大人口'
FROM countries
GROUP BY region;
...
SELECT countries.name AS 'name', countrylanguages.language AS 'language'
FROM countries
...
SELECT countries.name AS '国名', cities.name AS '市区町村名', countrylanguages.language AS 'language'
FROM countries
...
ON celebrities.country_code = countries.code
JOIN countrylanguages
ON celebrities.country_code = countrylanguages.country_code AND countrylanguages.is_official = 'T';
...
SELECT SUBSTRING(birth,1,4) AS '誕生年', count(*) AS '人数'
FROM celebrities
WHERE SUBSTRING(birth,1,4) = '1991'
GROUP BY SUBSTRING(birth,1,4)
UNION
SELECT SUBSTRING(birth,1,4), count(*)
FROM celebrities
